...
st.title('스트림릿에서의 오디오 표시 예')

st.subheader('1. 로컬 저장소의 오디오 재생')
audio_file = './data/서연의_하루_TTS_short.mp3'
audio_local = open(audio_file, 'rb')
audoio_bytes = audio_local.read()

st.text("MP3 파일. format='audio/mpeg'")
st.audio(audoio_bytes, format="audio/mpeg")
