...
print('극장 별 상영횟수 집계')
mygroupping = dftheater.groupby('theater')['bindo']
sumSeries = mygroupping.sum()
meanSeries = mygroupping.mean()
sizeSeries = mygroupping.size()

print('시리즈 3개를 이용한 데이터 프레임 생성')
df = pd.concat([sumSeries, meanSeries, sizeSeries], axis=1)
df.columns = ['합계', '평균', '개수']
print(df)
print("-" * 50)

mysize = len(mygroupping.groups)

df.plot(kind='bar', rot=0)
plt.title(str(mysize) + '개의 매장 집계 데이터')
...
print('집계 함수를 dict에 담아전달')
print('지역의 개수와 상영 횟수의 종합')
...
result = mygroupping.agg([np.count_nonzero, np.mean, np.std])
print(result)
print("-" * 50)
...
result = mygroupping.agg(myroot)
print(result)
print("-" * 50)

print('groupby와 사용자 정의 함수 사용 (매개변수 2개 사ㅛㅇ)')
result = mygroupping.agg(plus_add, somevalue=3)
print(result)
print("-" * 50)

print('columns 2개 이상을 그루핑 하기')
...
ax1.pie(sumSeries, labels=mytuple, explode=explode, shadow=True, autopct='%1.2f%%', startangle=90)
