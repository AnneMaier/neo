...
app.get('/insert', async (req, res) => {
    ...
    const result = await pool.query("insert into st_info values (?, ?, ?)",
        [st_id, name, dept]);
        res.redirect('/select');
})
...
app.get('/update', async (req, res) => {
    const { st_id, name, dept } = req.query;
    const result = await pool.query("update st_info set NAME = ?, DEPT = ? where ST_ID = ?",
        [name, dept, st_id]);
        res.redirect('/select');
})
...
app.get('/delete', async (req, res) => {
    ...
    const result = await pool.query("delete from st_info where ST_ID = ?",
        [st_id]);
        res.redirect('/select');
})
